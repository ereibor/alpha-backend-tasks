...
For local development and tests, the service will fall back to the fake summarization provider whenever:

- `NODE_ENV === "test"`, or
- `GEMINI_API_KEY` is empty.

To use a real Gemini provider in the running application:

1. Sign up for Google AI Studio and create a **Gemini API key**.
...
3. Restart the NestJS service.

When `GEMINI_API_KEY` is set and `NODE_ENV` is not `"test"`, the application will use the `GeminiSummarizationProvider` behind the `SummarizationProvider` interface. Jest tests continue to use the fake provider and never call the live API.
